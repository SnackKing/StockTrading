...
from .forms import SignupForm, LoginForm, NewClassForm, ResetPass
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.error(
                    request, 'No user exists with that username/password')
            ...
            if userid not in teachers:
                messages.error(
                    request, 'That account is not a teacher account')
                return redirect('teachers-login')
...
def studentList(request, joinCode):
    if 'tid' not in request.session:
        return redirect('teachers-login')
    tid = request.session['tid']
    classData = db.child('teachers').child(tid).child(
        'classes').child(joinCode).get().val()
    print(classData)
...
def leaderboard(request, joinCode):
    if 'tid' not in request.session:
        return redirect('teachers-login')
    tid = request.session['tid']
    students = db.child('teachers').child(tid).child(
        'classes').child(joinCode).child('students').get().val()
    leaderboard = {}
    for studentId, name in students.items():
        studentVal = round(getPortfolioValue(studentId), 2)
        leaderboard[studentId] = (studentVal, name)
    orderedKeys = sorted(leaderboard.keys(),
                         key=lambda x: leaderboard[x][0], reverse=True)
...
def getOwnedEquity(user):
    ...
    print(result)
    for item in result:
        equitys[item['symbol']] = round(
            float(item['lastSalePrice']) * int(user['owned'][item['symbol']]), 2)
    return equitys
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                return redirect('teachers-newpass')

    form = ResetPass()
    return render(request, 'teachers/newpassword.html', {'form': form})